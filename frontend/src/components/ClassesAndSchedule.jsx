      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <h2 className="text-4xl sm:text-6xl font-black italic tracking-tighter uppercase mb-4">
            HIGH-OCTANE <span className="text-[#B9FF00]">SCHEDULE.</span>
          </h2>
          <p className="text-white/50 text-base sm:text-lg font-light">
            Train alongside master coaches in our specialized athletic pods. Reserve your spot online with live slot tracking.
          </p>
        </div>
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                  isSelected
                    ? 'bg-[#B9FF00]/10 border border-[#B9FF00] text-[#B9FF00]'
        ...
        {filteredClasses.length === 0 ? (
          <div className="text-center py-16 bg-slate-950/50 rounded-2xl border border-slate-800">
            <p className="text-slate-400 text-base">No classes scheduled for {selectedDay} in {selectedCategory}.</p>
            <button
              onClick={() => {
                setSelectedCategory('All');
              }}
              className="mt-4 px-4 py-2 rounded-xl bg-slate-800 text-lime-400 text-xs font-bold cursor-pointer"
            >
              Show All Categories
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredClasses.map((cls) => {
              ...
              return (
                <div
                  key={cls.id}
                  className="bg-zinc-950 border border-white/10 rounded-3xl overflow-hidden hover:border-[#B9FF00]/50 transition-all duration-300 flex flex-col group shadow-2xl"
                >
                  ...
                  <div className="relative aspect-video overflow-hidden">
                    ...
                    {/* Category Badge */}
                    <div className="absolute top-3 left-3 bg-black/80 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-extrabold uppercase tracking-wider text-[#B9FF00] border border-white/10">
                      {cls.category}
                    </div>
                    ...
                    <button
                      ...
                      className="absolute top-3 right-3 p-2.5 rounded-full bg-black/80 backdrop-blur-md text-white hover:text-[#B9FF00] hover:scale-110 transition-all cursor-pointer border border-white/10"
                    ...
                    <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between text-xs font-semibold text-white/80">
                      <span className="flex items-center gap-1">
                        <Clock className="w-3.5 h-3.5 text-[#B9FF00]" /> {cls.time} ({cls.durationMinutes} min)
                      </span>
                      <span className="flex items-center gap-1 text-[#B9FF00] font-bold">
                  ...
                  <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                    <div>
                      <h3 className="text-xl font-black italic uppercase text-white mb-2 group-hover:text-[#B9FF00] transition-colors">
                        {cls.name}
                      </h3>
                    ...
                    <div className="pt-3 border-t border-white/10 space-y-2">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="w-full h-1.5 rounded-full bg-black overflow-hidden">
                          <div
                            className="h-full bg-[#B9FF00] rounded-full"
                    ...
                    <button
                      ...
                      className="w-full py-3 rounded-none bg-white/5 border border-white/10 text-[#B9FF00] font-black text-xs hover:bg-[#B9FF00] hover:text-black transition-all flex items-center justify-center gap-2 cursor-pointer uppercase tracking-widest"
                    >
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 relative shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            {!bookingSuccess ? (
              <form onSubmit={handleSubmitBooking} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full py-3 rounded-xl bg-lime-400 text-slate-950 font-bold text-sm uppercase tracking-wider hover:bg-lime-300 shadow-lg shadow-lime-400/20 transition-all cursor-pointer mt-4"
                >
                  Confirm Reservation
                </button>
              </form>
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <p className="text-xs text-slate-300 leading-relaxed">
                  We sent a confirmation pass to <span className="text-lime-400 font-semibold">{formData.email}</span>. See you on {selectedDay} at {bookingClass.time}!
                </p>
                <button
                  onClick={() => setBookingClass(null)}
