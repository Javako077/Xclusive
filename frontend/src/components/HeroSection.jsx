...
export const HeroSection = () => {
  ...
  return (
    <section id="hero" className="relative min-h-screen pt-28 pb-20 flex items-center bg-black overflow-hidden select-none">
      {/* Sliding Background Images Only */}
          ...
          return (
            ...
            >
              <img
                ...
                className="w-full h-full object-cover filter brightness-95 contrast-105 opacity-95 blur-[1px]"
              />
            </div>
          );
        })}

        {/* Soft Ambient Gradient Overlays for Maximum Background Clarity */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
        <div className="max-w-3xl space-y-8">
          ...
          {/* Key Stats Bar */}
          <div className="pt-10 grid grid-cols-2 sm:grid-cols-4 gap-6 border-t border-white/10 max-w-2xl">
      ...
      {/* Floating Bottom-Right Background Slide Controls */}
      <div className="absolute bottom-8 right-8 z-20 flex items-center gap-4 bg-black/60 backdrop-blur-md p-3 rounded-2xl border border-white/10 shadow-2xl">
        <button
          onClick={handlePrev}
          className="p-2.5 rounded-xl bg-white/5 border border-white/10 text-white/70 hover:text-white hover:bg-[#D4AF37] hover:text-black hover:border-[#D4AF37] transition-all cursor-pointer"
          ...
          {HERO_SLIDES.map((_, index) => (
            <button
              ...
              className={`h-2 rounded-full transition-all cursor-pointer ${
                index === currentSlide
                  ? 'w-8 bg-gradient-to-r from-[#F5D76E] to-[#D4AF37] shadow-[0_0_10px_#D4AF37]'
