...
export const MembershipCalculator = ({ onOpenFreePass }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center max-w-3xl mx-auto mb-14">
          ...
          <h2 className="text-4xl sm:text-6xl font-black italic tracking-tighter uppercase mb-4">
            CHOOSE YOUR <span className="text-[#B9FF00]">PASS.</span>
          </h2>
          <p className="text-white/50 text-base sm:text-lg font-light">
            No long-term contracts. Pause or cancel anytime. Tailor your package with custom recovery and coaching add-ons.
          </p>
        </div>
        ...
        <div className="flex justify-center items-center gap-4 mb-16">
          ...
          >
            <div
              className={`w-6 h-6 rounded-full bg-[#B9FF00] transition-transform duration-300 ${
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="px-2.5 py-1 rounded-full bg-[#B9FF00]/10 text-[#B9FF00] border border-[#B9FF00] text-[9px] font-black uppercase tracking-widest">
              SAVE 20%
            </span>
          </div>
        </div>
            ...
            return (
              <div
                ...
                className={`rounded-3xl p-8 border transition-all duration-300 flex flex-col justify-between cursor-pointer relative ${
                  isSelected
                    ? 'bg-zinc-950 border-[#B9FF00] shadow-2xl scale-[1.03]'
                    : 'bg-zinc-950/40 border-white/5 hover:border-white/20'
                }`}
              >
                {/* Popular Badge */}
                {plan.badge && (
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-[#B9FF00] text-black font-black text-[10px] uppercase tracking-widest px-4 py-1 rounded-full shadow">
                    {plan.badge}
                  </div>
                )}
                ...
                <div>
                  ...
                  <div className="mb-6 flex items-baseline gap-1">
                    <span className="text-4xl font-black italic text-[#B9FF00]">${displayPrice}</span>
                  ...
                  <ul className="space-y-3 mb-8">
                    {plan.features.map((feat, i) => (
                      <li key={i} className="flex items-start gap-2.5 text-xs text-white/70">
                        <Check className="w-4 h-4 text-[#B9FF00] shrink-0 mt-0.5" />
                        <span>{feat}</span>
                      </li>
                    ))}
                  </ul>
                ...
                <button
                  ...
                  className={`w-full py-3.5 font-black text-xs uppercase tracking-widest transition-all cursor-pointer ${
                    isSelected
                      ? 'bg-[#B9FF00] text-black hover:scale-105 shadow-xl shadow-[#B9FF00]/10'
        ...
        <div className="max-w-4xl mx-auto bg-zinc-950 border border-white/10 rounded-3xl p-6 sm:p-10 shadow-2xl">
          <div className="flex items-center gap-2 text-[#B9FF00] text-xs font-bold uppercase tracking-wider mb-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            <label
              className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                ptAddon ? 'bg-white/10 border-[#B9FF00]' : 'bg-black border-white/10'
              }`}
            >
              <div>
                <div className="text-xs font-bold text-white uppercase tracking-wider">2x PT Sessions</div>
                <div className="text-[11px] text-white/40">+$40 / mo</div>
              </div>
              <input
                type="checkbox"
                checked={ptAddon}
                onChange={(e) => setPtAddon(e.target.checked)}
                className="accent-[#B9FF00] w-4 h-4 cursor-pointer"
              />
            </label>

            <label
              className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                saunaAddon ? 'bg-white/10 border-[#B9FF00]' : 'bg-black border-white/10'
              }`}
            >
              <div>
            ...
            <label
              className={`p-4 rounded-2xl border flex items-center justify-between cursor-pointer transition-all ${
                dietAddon ? 'bg-white/10 border-[#B9FF00]' : 'bg-black border-white/10'
              }`}
            >
              <div>
          ...
          <div className="pt-6 border-t border-white/10 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <div className="text-3xl font-black italic text-[#B9FF00] mt-1">
            ...
            <button
              ...
              className="px-8 py-4 bg-[#B9FF00] text-black font-black text-xs uppercase tracking-widest hover:scale-105 shadow-xl shadow-[#B9FF00]/10 transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
              <Flame className="w-4 h-4 fill-black" /> Claim 3-Day Free Pass <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </div>
